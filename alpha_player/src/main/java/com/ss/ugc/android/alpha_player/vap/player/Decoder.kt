...
import com.ss.ugc.android.alpha_player.vap.util.SpeedControlUtil


abstract class Decoder(val player: AnimPlayer) : IAnimListener {

    companion object {
        private const val TAG = "dq.Decoder"
    ...
    fun preparePlay(videoWidth: Int, videoHeight: Int) {
        ...
        player.preparedListener?.onPrepared()
    }
    ...
    override fun onVideoRender(frameIndex: Int, config: AnimConfig?) {
        ...
        player.firstFrameListener?.onFirstFrame()
    }

    override fun onVideoComplete() {
        ALog.i(TAG, "onVideoComplete")
        player.animListener?.onVideoComplete()
        player.completionListener?.onCompletion()
    }
